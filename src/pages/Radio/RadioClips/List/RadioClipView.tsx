...
import RadioClip_Columns from './RadioClipColumn';
...
const RadioClipView: React.FC<Props> = ({ data, handlePageChange, page, handlePageSize }) => {
    ...
    <ListWrapper>
      <Card $fullHeight>
        <TopContainer>
          ...
          <RightContainer>
            <Space>
              <Link type="link" to={ROUTES.RADIO.CREATE}>
                Create Radio Clip
              </Link>
            </Space>
          </RightContainer>
        ...
        <TableContainer>
          <Table
            // loading={isLoading}
            data={data}
            columns={RadioClip_Columns({ handleEdit })}
            pagination={{
              current: page,
